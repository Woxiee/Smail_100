...
- (void)sd_setText:(NSString *)text
{
   ...
   // [self sd_setText:text];
    if (
        (!([text class] == [NSNull class]) && (![text isKindOfClass:[NSString class]]))
        || [text isEqualToString:@""]
        || (text == nil)
        || [text isEqualToString:@"<null>"]
        || [text isEqualToString:@""]
        || [text isKindOfClass:[NSNull class]]
        || [text isEqualToString:@"<nil>"]
        || [text isEqualToString:@"(null)"]
        ){
        
        [self sd_setText:@""];
    }else if([text containsString:@"(null)"]){
        NSString *aText = [text stringByReplacingOccurrencesOfString:@"(null)" withString:@""];
        [self sd_setText:aText];
    }else{
        [self sd_setText:text];
    }
