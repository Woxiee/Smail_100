...
-(void)addSubSegmentView
{
    ...
    self.lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-1.5, width, 1.5)];
...
-(void)tapAction:(id)sender{
    ...
    [UIView animateWithDuration:0.2 animations:^{
       self.lineImageView.frame = CGRectMake(button.frame.origin.x, self.frame.size.height-1.5, button.frame.size.width, 1.5);
    }];
...
-(void)selectIndex:(NSInteger)index
{
    for (UIButton *subButton in self.buttonsArray) {
        if (index != subButton.tag) {
            ...
            subButton.selected = YES;
            [UIView animateWithDuration:0.2 animations:^{
                self.lineImageView.frame = CGRectMake(subButton.frame.origin.x, self.frame.size.height-1.5, subButton.frame.size.width,1.5);
            }];
        }
    }
}
...
{
    _titles = titles;
    if (self.buttonsArray.count != _titles.count) {
        LOG(@"SBSBSB 这都能搞错 你是不是傻啊！");
        return;
    }
