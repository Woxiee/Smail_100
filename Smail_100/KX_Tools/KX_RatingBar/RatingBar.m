...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    [_s2 setFrame:CGRectMake(sigleWidth+5,     0, sigleWidth, sigleWidth)];
    [_s3 setFrame:CGRectMake(2 * (sigleWidth +5), 0, sigleWidth, sigleWidth)];
    [_s4 setFrame:CGRectMake(3 * (sigleWidth +5), 0, sigleWidth, sigleWidth)];
    [_s5 setFrame:CGRectMake(4 * (sigleWidth +5), 0, sigleWidth, sigleWidth)];
...
-(void)displayRating:(float)rating{
    ...
    starRating = rating;
    lastRating = rating;
    [_delegate ratingChanged:rating withRating:self];
}
...
- (void)changePostionForTouches:(NSSet *)touches {
    ...
    int newRating = (int) (point.x / self.frame.size.height);
    if (newRating > 5)
        return;
