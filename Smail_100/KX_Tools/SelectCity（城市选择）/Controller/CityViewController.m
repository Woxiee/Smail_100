...
- (void)viewDidLoad {
    ...
    _tableView.sectionIndexColor = DETAILTEXTCOLOR;
...
-(void)loadData
{
    ...
    if (KX_NULLString(self.currentCityString)) {
        self.currentCityString = @"深圳";
    }

    self.currentCityArray = @[self.currentCityString];
     self.hotCityArray = @[@"全国",@"上海",@"北京",@"广州",@"深圳",@"武汉",@"天津",@"西安",@"南京",@"杭州",@"成都",@"重庆"];
