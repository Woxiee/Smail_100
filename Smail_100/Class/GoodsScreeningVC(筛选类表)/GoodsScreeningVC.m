...
{
    WEAKSELF;
    if (KX_NULLString(_seachClassId)) {
        _seachClassId = @"";
    }
    ...
    [param setObject:_transactionType forKey:@"transactionType"];
    [param setObject:_orderBy forKey:@"orderBy"];
    [param setObject:_keyWord forKey:@"keyWord"];
    [param setObject:_seachClassId forKey:@"seachClassId"];
    [param setObject:_paymentType forKey:@"paymentType"];

    if ([[KX_UserInfo sharedKX_UserInfo].city isEqualToString:@"全国"] || KX_NULLString([KX_UserInfo sharedKX_UserInfo].city)) {
        _region = @"";
    }else{
        _region = [KX_UserInfo sharedKX_UserInfo].city;
    }
    
    if (_isSupply) {
        [param setObject:[KX_UserInfo sharedKX_UserInfo].bid forKey:@"bid"];
        if (KX_NULLString(_parentClassId)) {
            _parentClassId = @"";
        }
        _region = @"";
        [param setObject:_parentClassId forKey:@"parentClassId"];
    }
    
    [param setObject:_region forKey:@"region"];
    ...
    [GoodsScreenVmodel getGoodsScreenListParam:param WithDataList:^(NSArray *dataArray, BOOL success) {
        ...
        if (success) {
            ...
            [weakSelf.sourceData addObjectsFromArray:dataArray];
