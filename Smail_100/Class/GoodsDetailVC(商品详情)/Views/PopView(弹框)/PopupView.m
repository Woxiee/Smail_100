...
@property (nonatomic, strong) UIImageView *popImageBgView;
...
- (instancetype)init {
    if (self = [super init]) {
        self.rows = 0;
        self.Triangle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tishikuang@3x.png"]];
...
- (UIImageView *)popImageBgView
{
    if (!_popImageBgView) {
        _popImageBgView = [[UIImageView alloc] init];
        _popImageBgView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        _popImageBgView.image = [UIImage imageNamed:@"tishikuang@3x.png"];
    }
    
    return _popImageBgView;
}
...
+ (void)show {
    ...
    popupView.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - popupViewWidth - popupViewpadding, 66, popupViewWidth, rowHeight * popupView.rows);
    
    [popupView addSubview: popupView.popImageBgView];

    [popupView addSubview: popupView.tableView];
   popupView.tableView.backgroundColor = [UIColor clearColor];
