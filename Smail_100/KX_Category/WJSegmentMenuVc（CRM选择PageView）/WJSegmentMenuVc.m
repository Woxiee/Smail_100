...
- (void)initScrollviewWithTitles:(NSArray *)titles{
    ...
    for (int i = 0; i < titles.count; i ++) {
        ...
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(titles.count <5?(SCREEN_WIDTH/titles.count)*i: WJSegmentMenuVcDefaultSpace + (WJSegmentMenuVcDefaulButtonW + WJSegmentMenuVcDefaultSpace) * i, WJSegmentMenuVcDefaultSpace, titles.count <5?SCREEN_WIDTH/titles.count:WJSegmentMenuVcDefaulButtonW, self.frame.size.height - WJSegmentMenuVcDefaultSpace * 2)];
...
- (void)segmentMenuTitleClick:(UIButton *)btn{
    ...
    if (self.didClickPageIndexBlock) {
        self.didClickPageIndexBlock(btn.tag - tag);
    }
}
...
- (void)buttonMoveWithIndex:(NSInteger)clickIndex{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        tempFrame.origin.x = self.vcArray.count<5?(SCREEN_WIDTH/self.vcArray.count)
        *clickIndex:WJSegmentMenuVcDefaultSpace + (WJSegmentMenuVcDefaulButtonW + WJSegmentMenuVcDefaultSpace) * clickIndex;
