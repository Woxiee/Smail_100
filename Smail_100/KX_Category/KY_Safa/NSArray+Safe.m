...
- (void)ls_removeObjectAtIndex:(NSUInteger)index
{
    if (index >= [self count])
    {
        showAlertMasse(@"removeObjectAtIndex:");
        return;
    }
...
- (void)ls_replaceObjectAtIndex:(NSUInteger)index withObject:(id)anObject
{
    if (index >= [self count])
    {
        showAlertMasse(@"replaceObjectAtIndex: withObject:");
        return;
    }
...
- (void)ls_removeObjectsInRange:(NSRange)range
{
    ...
    {
        showAlertMasse(@"removeObjectsInRange:");
        return;
    }
...
- (void)ls_insertObject:(id)anObject atIndex:(NSUInteger)index
{
    if (index > self.count)
    {
        showAlertMasse(@"insertObject: atIndex:");
        return;
    }
